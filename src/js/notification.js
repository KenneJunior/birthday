export default class Notification {
  constructor(options = {}) {
    ...
    for (const [key, id] of Object.entries(this.config.elementIds)) {
      ...
      if (
        key === "showBtn" &&
        !this.element[key] &&
        typeof this.config.elementIds.showBtn === "string"
      ) {
        this.element[key] = document.querySelector(
          this.config.elementIds.showBtn
        );
      }
    }
  ...
  setNotificationType(type, customData = {}) {
    ...
    const iconElement = this.element.notification.querySelector(
      ".notification-icon i"
    );
  ...
  setupEventListeners() {
    ...
    if (this.element.showBtn) {
      this.element.showBtn.addEventListener("click", () =>
    ...
    if (this.element.closeBtn) {
      this.element.closeBtn.addEventListener("click", () =>
        this.hideNotification()
      );
    }
    ...
    if (this.element.dismiss) {
      this.element.dismiss.addEventListener("click", () =>
        this.hideNotification()
      );
    }
    ...
    document.addEventListener("click", (event) => {
      if (
        this.element.notification.classList.contains("visible") &&
